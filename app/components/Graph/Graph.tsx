...
const Graph = ({graphData, physics, setPhysics, visuals, setVisuals, previewNode, setPreviewNode }: GraphProps) => {
  
  const [width, height] = useWindowSize();
  
  // Create a state variable to store the filter parameters
  [physics, setPhysics] = useState(initialPhysics);
  // Create a state variable to store the visual parameters
  [visuals, setVisuals] = useState(initialVisuals);
  // Create a state variable to store the node that is currently being previewed
  ...
  const handleClick = useCallback(
    (node: NodeObject) => setPreviewNode(node),
    [fgRef]
  );
  ...
  return (
    <>
      ...
      <NodePanel previewNode={previewNode}/>
